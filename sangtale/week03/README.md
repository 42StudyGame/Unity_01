...
본격적인 게임 개발! 예제 프로그램을 다운 받아 구동 해봤다. <br/>
탄막을 피해 원통을 움직여서 오래 살아남으면 되는 게임이다. <br/>
simple is best, 간단하지만 재미난 구성이다. <br/>

* **6장에서 해본 것**
    1. Inspector의 Transform으로 컴포넌트의 스케일과 위치를 수정하기
        - Plane의 크기와 유닛 단위
            유니티에서 1 유닛은 Cube 한 변의 길이입니다. Plane의 크기는 가로 x 세로 10 유닛입니다.<br/>
            그러므로 Plane에는 큐브가 100개가 들어갈 수 있습니다.<br/>
            1 유닛을 현실의 몇 미터에 대응할지는 개발자 마음이지만, 보통 편의상 1유닛을 1미터로 취급합니다.<br/>
    2. 머티리얼(Material)
        - 머티리얼은 Shader와 Texture가 합쳐진 에셋으로, 오브젝트의 픽셀 컬러를 결정합니다.
        - 알베도(Albedo)는 반사율이라는 뜻으로, 물체가 어떤 색을 반사할지 결정합니다. 알베도의 RGB 값을 수정해서 물체의 기본색을 결정합니다.
    3. 여러 가지 오브젝트 만들고, 카메라 설정하기
        - 바닥과 벽, 그리고 원통 모양의 플레이어를 만듭니다. 이에 머티리얼을 입혀 색깔을 지정합니다.
        - 빈 오브젝트를 만들어 여러 오브젝트를 담을 부모 프로젝트로 지정합니다. 복잡해진 하이어라키창을 정리할 때 좋습니다.
        - 카메라 위치를 설정하고 디폴트 설정인 스카이 박스(Skybox)를 단색(Solid Color)으로 바꿉니다.
        - 캡슐 오브젝트를 만들어 플레이어로 지정하고 푸른색의 머티리얼을 입힙니다.
    4. 오브젝트 설정하기
        - 이후에 탄알과 플레이어가 충돌했는지 확인하기 위해 플레이어 오브젝트에 태그(Tag)를 자기 자신으로 추가합니다.
            > 이렇게 태그를 설정하면 나중에 탄알이 플레이어와 충돌했을 때만 게임오버가 실행되게 할 수 있습니다. 
        - 플레이어에 Rigidbody 컴포넌트를 추가하여 물리적인 효과를 줍니다.
            > Rigidbody의 제약(Constraints)을 추가로 설정합니다. y축 Position을 고정하지 않으면 벽을 뛰어 넘어서고, x, z축 Rotation을 고정하지 않으면 오뚝이처럼 뒤뚱뒤뚱 합니다. play 버튼을 누른 후 체크를 해제하면 바로 확인 가능!
        - 키 입력을 받아 움직이는 c# 스크립트를 작성하고, 플레이어 오브젝트의 컴포넌트에 추가합니다.
    5. 키 입력 스크립트
        - Unity 프로그램은 Update() 메소드를 통해 실시간으로 변하는 정보를 주기적으로 체크합니다. 따라서 이 메소드를 수정하여 키를 입력 받고 오브젝트를 실시간으로 제어합니다.
        - Input() 메소드를 통해 키 입력을 감지합니다. 대략 1초에 60번 정도 키 입력을 감지합니다.
        - 큐브를 점프시켰던 Rigidbody.AddForce() 메소드와 Input.GetKey()를 통해 상하좌우 방향키에 따라 힘을 주는 식으로 코드를 짜보고,
        - Input.GetAxis()와 Vector3, Rigidbody.velocity를 통해 움직임을 개선해봅니다.
            > **AddForce()와 velocity의 차이?**
                - Addforce()는 서서히 속도를 증감시켜 관성의 영향을 받고, velocity에 새로운 속도를 덮어씌우는 것이라 관성을 무시합니다.
        - Edit > Project Settings... > Input > Axes에서 Axes 리스트를 훑어봅니다. 이후에 키 코드를 수정할 수 있습니다. 멀티 플랫폼 또한 지원합니다. (멘토님의 xbox cotroller의 얘기가 바로 이 부분...)
        - 입력을 boolean형이 아닌 숫자로 받는 이유는, 멀티플랫폼에 대응하기 위해서 입니다. 아날로그 스틱의 경우, 스틱을 절반만 밀 경우에 서서히 움직이는 기능을 사용할 수 있습니다.
